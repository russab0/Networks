    ...
    while (1) {
        ...
        if (FD_ISSET(master_sock_tcp_fd, &readfds)) {
            ...
            while (1) {
                 ...
                 * for forever*/
                /*Step 9 */
                if (client_data-> a[0] == '0' && client_data->b == 0 && client_data->b == 0) {
		...
		strcpy(result.c, client_data->a);
		int cur = strlen(result.c);

		result.c[cur] = ' ';
		cur += 1;
		
		int b = (int) client_data->b;
		while (b != 0){
			result.c[cur] = (char) (b % 10 + (int) '0');		
			b /= 10;
			cur += 1;					
		}	
		result.c[cur] = ' ';
		...
		int c = (int) client_data->c;
		while (c != 0){
			result.c[cur] = (char) (c % 10 + (int) '0');		
			c /= 10;
			cur += 1;				
		}
